...
namespace Crystal
{
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		CR_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders at the moment.");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				CR_CORE_ERROR("OpenGL Shader compilation error.");
		...
		if (isLinked == GL_FALSE)
		{
			...
			CR_CORE_ERROR("OpenGL Shader linking error:");
			CR_CORE_ERROR("{0}", infoLog.data());
